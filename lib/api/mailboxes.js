...
module.exports = (db, server, mailboxHandler) => {
    ...
    server.post(
        {
            path: '/users/unified/mailboxes',
            summary: 'Request all Mailbox information',
            tags: ['Mailboxes'],
            validationObjs: {
                requestBody: {
                    users: Joi.array().items(Joi.string().hex().lowercase().length(24).required().description('IDs of Users')),
                    mailboxes: Joi.array().items(Joi.string().hex().lowercase().length(24).optional().description('IDs of Mailboxes')).optional()
                },
                queryParams: {
                    sess: sessSchema,
                    ip: sessIPSchema
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            try {
                ...
                console.log(mailboxQuery);
                total = await db.database.collection('messages').countDocuments(mailboxQuery);
            } catch (err) {
                // ignore
            }

            try {
                mailboxQuery.unseen = true
                console.log("Unseen count query");
                console.log(mailboxQuery);
                unseen = await db.database.collection('messages').countDocuments(mailboxQuery);
            } catch (err) {
                // ignore
            }
